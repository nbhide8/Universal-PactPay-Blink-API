...
// ---------------------------------------------------------------------------
const BLINK_API_URL =
  process.env.BLINK_API_URL ?? "http://localhost:3001/api/v1";
...
server.tool(
  "browse_rooms",
  "Browse public escrow rooms. Supports filtering by status, a search string, tags, and pagination.",
  {
    status: z
      .enum([
        "pending",
...
server.tool(
  "create_room",
  `Create a new escrow room backed by a Solana on-chain PDA.
...
server.tool(
  "get_room",
  "Fetch full details for a single room including live on-chain PDA state (staked balances, resolution approvals, etc.).",
  {
...
server.tool(
  "join_room",
  "Join an existing escrow room using its join code. Returns a lockbox action the joiner may need to sign (direct mode) or a Stripe confirmation (custodial).",
  {
    walletAddress: z
      .string()
...
server.tool(
  "stake_room",
  `Fund the creator or joiner side of the escrow.
...
server.tool(
  "submit_transaction",
  `Submit a base64-encoded signed Solana transaction to the network. Use this after the user signs a transaction returned by create_room or stake_room.
...
server.tool(
  "approve_room",
  "Signal that a participant approves resolving the escrow. Both creator and joiner must call this before resolve_room can release funds.",
  {
    roomId: z.string().describe("UUID of the room"),
...
server.tool(
  "resolve_room",
  "Resolve the escrow — releases staked SOL back to both participants on-chain. Requires both parties to have called approve_room first.",
  {
    roomId: z.string().describe("UUID of the room"),
...
server.tool(
  "slash_room",
  "Slash the escrow — sends all staked SOL to the platform penalty wallet instead of returning it. Use when a party has violated the contract terms.",
  {
    roomId: z.string().describe("UUID of the room"),
    walletAddress: z.string().describe("Wallet address of the caller"),
  },
...
server.tool(
  "cancel_room",
  "Cancel a room before it is fully funded. Only the room creator can cancel, and only while the room is still in pending/funding status.",
  {
    roomId: z.string().describe("UUID of the room"),
    walletAddress: z
      .string()
...
server.tool(
  "manage_events",
  `Control the Solana on-chain event listener that watches the Anchor program for escrow events and triggers automatic actions (e.g. Stripe captures after successful stakes).
