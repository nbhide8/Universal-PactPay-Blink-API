  ...
  lockbox: {
    ...
    action: {
      ...
      },
      {
        ...
        description:
          'Stripe webhook endpoint. Receives payment events and auto-stakes SOL on-chain when a fiat deposit is confirmed. ' +
          'No API key needed — uses Stripe signature verification. Configure in Stripe Dashboard → Webhooks.',
        authentication: 'Stripe webhook signature (STRIPE_WEBHOOK_SECRET)',
        events: [
          'payment_intent.amount_capturable_updated — Deposit authorized → auto-stake SOL on-chain',
          'payment_intent.payment_failed — Mark deposit as failed',
          'payment_intent.canceled — Mark deposit as refunded',
        ],
      },
      {
        method: 'GET',
        path: '/api/v1/events',
        description: 'Get on-chain event listener status (running, processedCount, etc.).',
        response: '{ success, listener: { running, processedCount, lastSignature, handlerCount, dispatcherRegistered } }',
      },
      {
        method: 'POST',
        path: '/api/v1/events',
        description:
          'Manage the on-chain event listener. Actions: start (begin polling Solana for program events + register Stripe dispatcher), ' +
          'stop (halt polling), poll (one-shot manual poll, returns discovered events).',
        body: {
          action: { type: 'string', required: true, enum: ['start', 'stop', 'poll'] },
          intervalMs: { type: 'number', default: 10000, description: 'Polling interval (for start action)' },
        },
