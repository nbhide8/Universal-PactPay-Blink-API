...
import { createDeposit } from '@/lib/stripe/escrow';
import { recordDeposit } from '@/lib/stripe/deposits';
import { isStripeAvailable } from '@/lib/stripe/client';
import { computeTermsHash } from '@/lib/crypto/termsHash';
import { getRoomEscrowPDA } from '@/lib/solana/pda';
...
export class CustodialEngine implements EscrowEngine {
   ...
   * Stripe → create REAL PaymentIntent with manual capture, return client_secret
   * Credits → instant, no client action needed
   */
  private async fiatAction(
    ...
    userId: string,
    escrowPda?: string
  ): Promise<{ type: 'confirm_payment' | 'none'; payload: string | null; instructions: string; metadata: Record<string, any> }> {
    if (rail === 'stripe') {
      if (!isStripeAvailable()) {
        // Fallback to mock if Stripe keys not set (dev mode)
        const mockClientSecret = `pi_${roomId.slice(0, 8)}_${role}_secret_demo`;
        return {
          type: 'confirm_payment',
          payload: mockClientSecret,
          instructions:
            `[DEV MODE] Confirm this mock PaymentIntent. In production, set STRIPE_SECRET_KEY ` +
            `for real fiat deposits of ${amount} ${currency}.`,
          metadata: { amount, currency, captureMethod: 'manual', devMode: true },
        };
      }

      // REAL Stripe PaymentIntent with manual capture (funds held, not charged)
      ...
      // Record in our deposits table for event listener to find later
      await recordDeposit({
        roomId,
        userId,
        role,
        stripePaymentIntentId: deposit.paymentIntentId,
        amountCents,
        currency,
        escrowPda,
      });
      ...
      return {
        type: 'confirm_payment',
        payload: deposit.clientSecret,
        instructions:
          `Confirm this Stripe PaymentIntent to deposit ${amount} ${currency}. ` +
          'Use stripe.confirmCardPayment(clientSecret) on the frontend. ' +
          'Funds will be HELD (not charged) until the on-chain escrow resolves. ' +
          'The blockchain event triggers the actual capture or refund.',
        metadata: {
          ...
          holdDuration: '7 days (extendable to 31)',
          note: 'Funds held by Stripe. On-chain resolve → capture. On-chain cancel → refund.',
        },
      };
    }
  ...
  async createLockbox(params: CreateLockboxParams): Promise<LockboxResult> {
    ...
    const fiatResult = await this.fiatAction(
      rail,
  ...
  async fundLockbox(params: FundLockboxParams): Promise<LockboxResult> {
    ...
    const fiatResult = await this.fiatAction(rail, params.amount, currency, params.roomId, role as 'creator' | 'joiner', params.funderId, undefined);
