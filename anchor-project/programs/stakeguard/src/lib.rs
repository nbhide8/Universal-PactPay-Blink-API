...
pub mod stakeguard {
    ...
    pub fn stake(
        ...
        // Save values for the event before dropping borrows
        let final_total_staked = escrow.total_staked;
        let final_is_fully_funded = escrow.is_fully_funded;
        ...
        emit!(Staked {
            ...
            amount,
            is_creator,
            total_staked: final_total_staked,
            is_fully_funded: final_is_fully_funded,
        });
    ...
    pub fn resolve(
        ...
        // Drop mutable borrows before lamport transfers
        drop(escrow);
        drop(creator_stake_record);
        drop(joiner_stake_record);

        // Transfer creator's stake back
        if creator_amount > 0 {
            let escrow_info = ctx.accounts.escrow_account.to_account_info();
            let creator_info = ctx.accounts.creator_wallet.to_account_info();
            **escrow_info.try_borrow_mut_lamports()? -= creator_amount;
        ...
        if joiner_amount > 0 {
            let escrow_info = ctx.accounts.escrow_account.to_account_info();
            let joiner_info = ctx.accounts.joiner_wallet.to_account_info();
            **escrow_info.try_borrow_mut_lamports()? -= joiner_amount;
            **joiner_info.try_borrow_mut_lamports()? += joiner_amount;
        }
    ...
    pub fn slash(
        ...
        let escrow_info = ctx.accounts.escrow_account.to_account_info();
        let penalty_info = ctx.accounts.penalty_wallet.to_account_info();
        **escrow_info.try_borrow_mut_lamports()? -= total_slash;
        **penalty_info.try_borrow_mut_lamports()? += total_slash;
    ...
    pub fn cancel_room(
        ...
        // Save amounts and reset before lamport transfers
        let creator_refund = escrow.creator_staked;
        let joiner_refund = escrow.joiner_staked;
        let has_joiner = escrow.joiner != Pubkey::default();
        escrow.creator_staked = 0;
        escrow.joiner_staked = 0;
        escrow.total_staked = 0;
        ...
        if creator_refund > 0 {
            ...
            **escrow_info.try_borrow_mut_lamports()? -= creator_refund;
            **creator_info.try_borrow_mut_lamports()? += creator_refund;
        }

        // Return any staked joiner funds (if joiner exists)
        if joiner_refund > 0 && has_joiner {
            if let Some(joiner_wallet) = &ctx.accounts.joiner_wallet {
                let escrow_info = ctx.accounts.escrow_account.to_account_info();
                let joiner_info = joiner_wallet.to_account_info();
                **escrow_info.try_borrow_mut_lamports()? -= joiner_refund;
                **joiner_info.try_borrow_mut_lamports()? += joiner_refund;
            }
        }
